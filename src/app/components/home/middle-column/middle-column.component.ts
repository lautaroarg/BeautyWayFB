...
export class MiddleColumnComponent implements OnInit {
  ...
  async OnChange(event?) {
    ...
    const MainImgSizes = await this.imageOptSrvc
      .AdjustImageHeightWidth(event.target.files[0], "Post")
      .toPromise();
    const ObservableImage =
    ...
    this.showImage = true;
    this.ImageCords = await this.PreviewLoaded(
      this.OptimizedImage.compressedImage.imageDataUrl
    ).toPromise();

    const UploadableBlob = await this.imageOptSrvc.dataURItoBlob(
      resImage.compressedImage.imageDataUrl
    );
    this.UploadableImage = UploadableBlob;
    const ThumbImgSizes = await this.imageOptSrvc
    ...
    const UploadableThumbBlob = await this.imageOptSrvc.dataURItoBlob(
      ThumbImage.compressedImage.imageDataUrl
    );
  ...
  UploadThePost() {
    ...
    const ImagePath: string =
      this.MyAuth.LoggedUser.Id +
      "/PostedImages/" +
      this.FileName +
      "-" +
      moment().format("D-M-YYYY");
    const ImageThumbPath: string =
      this.MyAuth.LoggedUser.Id +
      "/PostedImages/" +
      this.FileName +
      "-" +
    ...
    const ImageUploadTask = this.storage.upload(
      ImagePath,
      this.UploadableImage,
      { customMetadata: { Caption: Cap } }
    );
    const ThumbImageUploadTask = this.storage.upload(
      ImageThumbPath,
      this.UploadableThumbImage,
      { customMetadata: { PostImageThumbFor: ImagePath } }
    );
